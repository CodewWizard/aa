...
import { violationMasterData } from './constant';
...
const violationsInfoModel = [{
      ...
      controls: [
        ...
        },
        {
          ...
          options: [
            { key: "SelectaccidentWithViolation", label: "Please Select", value: "" },
            { key: "accidentWithViolationOption1", label: "No violation", value: "No violation" },
            { key: "accidentWithViolationOption2", label: "Acc At Fault", value: "Acc At Fault" },
            { key: "accidentWithViolationOption3", label: "Acc not at fault", value: "Acc not at fault" }
          ],
        },
        {
          key: "violationPoint",
...
const ViolationActions = (cell, row, rowIndex, formatExtraData) => {
  return (
    <>
      <a
        className={row.Source === "ENT" ? "display-inblock actionIcon me-1" : "display-inblock actionIcon me-1 disableIcon"}
        data-bs-toggle="tooltip"
        title="Edit"
        onClick={() => row.Source === "ENT" ? onTableEdit(row, rowIndex) : null}
      >
      ...
      <a
        ...
        data-bs-toggle="tooltip"
        title="Delete"
        onClick={() => row.Source === "ENT" ? onTableDelete(row, rowIndex) : null}
      >
...
const onChange = (changedSchema, key) => {
  ...
  if (key == "violationType") {
    ...
    if (changedSchema.Type != "") {
      ...
      let matchingViolations = violationsList.filter((violation) => violation.Type == filledType);
      if (matchingViolations?.length <= 0)
        changedSchema.TotalPoints = dataObj[0]?.FirstOffense;
      else
        changedSchema.TotalPoints = dataObj[0].SecondandMoreOffense;
    }
  }
...
const onTableAdd = (model) => {
  let tempObj = [];
  let lastIndex = tempObj?.length > 0 ? (parseInt(tempObj.slice(-1)[0].ViolationNumber) + 1) : 1;
...
export default {
    title: 'Design System/Core/AddDetails',
    component: AddDetail,
    argTypes: {
        formKey: {
        ...
        resetLabel: {
          description: "UI schema which Provide reset label to the button in modal.",
          table: {
            category: 'Element'
          }
        },
        saveLabel: {
          description: "UI schema which Provide save label to the button in modal.",
          table: {
            category: 'Element'
          }
        },
        ...
        onReset: {
          action: (model) => { console.log(model); },
            description: "Emits this event whenever reset the data model of the form.",
            table: {
                category: 'Events'
            },
        },
...
const Template = (args) => <AddDetail {...args} />;
