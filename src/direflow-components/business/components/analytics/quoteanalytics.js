...
import { QuoteChartsDTColummns, paginationOptions } from "./constants/constants";
...
const QuoteAnalyticsComponent = (props) => {
  ...
  const quoteAnalytics = analyticsConst.QuoteInitiatorAnalyticsConfig(policies, 2023);
  const [filtersFormData, setFiltersFormData] = useState(props.FiltersFormData);
  const [updatedPolicies, setUpdatedPolicies] = useState(quoteAnalytics);
  ...
  const quotePremium = {
    ...
    filterValue: (cell, row) => {
      let premiumelement = row.TransactionHistory?.filter(record => {
        return record.Type == props.transactionKeys.POLICY
      });
      let premium
      if (premiumelement?.length > 0) {
        premium = premiumelement[0]?.EffectivePremium;
      } else {
        premium = row.Premium
      }

      const premium2 = parseFloat(premium);
      const premiumFormatted = premium2.toLocaleString('en-US', {
        style: 'currency',
        currency: 'USD'
      });
      return typeof (premium) == "number" ? premiumFormatted : "-";
    },
    formatter: (cell, row) => {
      let premiumelement = row.TransactionHistory?.filter(record => {
        return record.Type == props.transactionKeys.POLICY
    ...
    sortFunc: (a, b, order, dataField, rowA, rowB) => {
      let premiumelementA = rowA.TransactionHistory?.filter(record => {
        return record.Type == props.transactionKeys.POLICY
      });
      let premiumA
      if (premiumelementA?.length > 0) {
        premiumA = premiumelementA[0]?.EffectivePremium;
      } else {
        premiumA = rowA.Premium
      }
      let premiumelementB = rowB.TransactionHistory?.filter(record => {
        return record.Type == props.transactionKeys.POLICY
      });
      let premiumB
      if (premiumelementB?.length > 0) {
        premiumB = premiumelementB[0]?.EffectivePremium;
      } else {
        premiumB = rowB.Premium
      }
      if (order === 'asc') {
        return parseInt(premiumA) - parseInt(premiumB);
      }
      return parseInt(premiumB) - parseInt(premiumA); // desc
    }
  }

  let ChartsDTColummns = [...QuoteChartsDTColummns, TableIndexObject, quotePremium]
  const [tableColumns, setTableColumns] = useState(ChartsDTColummns)
  useEffect(() => {
    debugger
    const filterPolicy = policies.filter(policy => {
      return (
        policy.IsFormalQuoteOffered == 1 && policy.IsUwData == false
      )
    })
    ...
    if (props?.tableConfigQA) {
      let columns = props?.tableConfigQA?.columns;
      let isOverrideColumns = props?.tableConfigQA?.overrideColumns;
  ...
  useEffect(() => {
    applyFilters();
    const filterConstant = chartFilters.find(x => x.tabName == 'policyanalytics' && x.chartName == quoteAnalytics.YearwiseCount.chartKey);
  ...
  const applyFilters = () => {
    ...
    if (tempPolicies?.length == 0) {
      ...
      let tempdata = updateFiltersFormData(filtersFormData, filters)
      setFiltersFormData(tempdata)
      return false;
    }
    setFilteredPolicies(tempPolicies);
    setUpdatedPolicies(analyticsConst.QuoteInitiatorAnalyticsConfig(tempPolicies, policyYear));
  }
  ...
  const TypeFilterPolicies = (chartName, keyName, keyValue) => {
    if (chartName == quoteAnalytics.YearwiseCount.chartKey) {
      setShowMonthly(true);
    ...
    <>
      <div className='row'>
        <div className='col-md-12'>
          <h5 className="primaryColor3 graphik-Medium pb-2">Quote Initators</h5>
        </div>
      </div>
...
export default QuoteAnalyticsComponent;